...
function renderData(data) {
  const studentsArr = data.students.map(s => new Student(s.id, s.name, s.age, s.course));
  const instructorsArr = data.instructors.map(i => new Instructor(i.id, i.name, i.subject));
  ...
  studentsArr.forEach(st => {
    ...
    li.innerHTML = `${st.name} (${st.age}) - ${st.course}${star ? `<span class="student-star">${star}</span>` : ''}`;
